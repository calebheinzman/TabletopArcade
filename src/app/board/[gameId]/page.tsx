...
function BoardContent() {
  ...
  const { gameState, revealedCard, subscribeToGame, unsubscribeFromGame } =
    useGame();

  useEffect(() => {
    subscribeToGame(params.gameId as string);
    return () => unsubscribeFromGame();
  }, [params.gameId, subscribeToGame, unsubscribeFromGame]);
      ...
      <div className="w-full max-w-4xl mx-auto flex flex-col h-[98vh]">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
          ...
          <div className="space-x-2 self-end">
            ...
            <Button onClick={handleGiveToken} size="sm">
              Give Token
            </Button>
            <Button onClick={handleDiscardCard} size="sm">
              Discard Card
            </Button>
          </div>
        </div>
